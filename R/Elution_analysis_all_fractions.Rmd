...
sparkline <- function(z){
    ...
    z <- z %>% group_by(experimentID) %>% mutate(speccounts = normalit(speccounts))
    
    #If there is one group 
    #plt <- ggplot(z, aes(x=FractionID, y=PeptideCount, group=1)) +
    plt <- ggplot(z, aes(x=FractionID, y=PeptideCount, group=ExperimentID)) +
...
```{R}
    ...
    print(head(exp_data))
    print(head(t(exp_data)))
    square <-melt(cor(t(exp_data)))
    0 -> square[is.na(square)]
...
```{r}
data <- read.csv("Tcell_2502_Cyto_sequence_annotated.csv", sep=",", header=TRUE) %>% spread(FractionID, PeptideCount) %>% gather(FractionID, PeptideCount, -ProteinID, -Peptide, -Start, -End, -Length,-Appearance, -ExperimentID, -PeptideArea, -Sequence)
...
#find the proteins with most unique peptides identified
 data %>% group_by(ProteinID) %>% summarize(Pep_number=length(unique(Peptide))) -> data2
#reorder this temporary data set by number of unique peptides
 data2$ProteinID <- reorder(data2$ProteinID,data2$Pep_number)
...
#sp|F4I7I0|ALAT1_ARATH  #sp|Q9FKA5|Y5957_ARATH #tr|F4KHD5|F4KHD5_ARATH #example proteins for A. thaliana
  for(protein_name in tail(data2[order(data.frame(data2$Pep_number)),]$ProteinID,5)){
  #for(protein_name in c('sp|Q9SI75|EFGC_ARATH','sp|O81283|TC159_ARATH','sp|Q9LSB4|NAI2_ARATH','sp|Q9SI75|EFGC_ARATH','sp|Q9XI36|MBD10_ARATH','sp|Q56YU0|AL2C4_ARATH','sp|Q38858|CALR2_ARATH','tr|F4ISU2|F4ISU2_ARATH','tr|F4J9K9|F4J9K9_ARATH','tr|F4KHD5|F4KHD5_ARATH')){   #'tr|Q8VZW6|Q8VZW6_ARATH')){ #test proteins
        ...
        print(paste('Processing',protein_name))
          #print(data_protein)
          data_protein$Peptide <- factor(data_protein$Peptide)
          ...
          data_wide <- spread(data_protein, FractionID, PeptideCount)
    ...
    data_wide2$ProteinID <- reorder(data_wide2$ProteinID,data_wide2$Start)
    ...
    exp_data <- as.matrix(pre_matrix)
          ...
          final_prot <- data_wide %>% gather(FractionID, PeptideCount, -ProteinID, -Peptide, -Start, -End, -Length,-Appearance, -Sequence, -ExperimentID, -PeptideArea)
...
title <-ggdraw()+draw_label(paste("Protein:",protein_name), vjust=0, size=20)
      ...
      hm <- heatmap(exp_data)
      mp <- metric_plot(exp_data)
      
      figure <-  plot_grid(clusterplot, coverage,heatmap(exp_data), metric_plot(exp_data), nrow=1)
      figure_with_title <- plot_grid(title,figure,nrow=2,rel_heights = c(.05,1))
      ggsave(filename=paste('figure_', protein_name,'.png',sep=''), width=16, height=10, units='in', plot=figure_with_title)
