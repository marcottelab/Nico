...
sparkline <- function(z){
    ...
    z2 <- z %>% group_by(experimentID) %>% mutate(speccounts = normalit(speccounts))
    plt <- ggplot(z2, aes(x=Fraction, y=Count, group=1)) +
    geom_line(size=0.5) +
...
```{r}
data <- read.csv("test_3exp.csv", sep=",", header=TRUE) %>% spread(Fraction, Count) %>% gather(Fraction, Count, -Protein, -Peptide, -Start, -End, -Length,-Appearance, -Sequence)
data[is.na(data_wide)] <- 0
```
...
title <-ggdraw()+draw_label(paste("Protein:",protein_name), vjust=0, size=20)
      ...
      ggsave(filename=paste('Test_multiexp/figure_', protein_name,'.png',sep=''), width=16, height=10, units='in', plot=figure_with_title)
